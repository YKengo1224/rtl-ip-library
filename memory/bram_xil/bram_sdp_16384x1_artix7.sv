...
module bram_sdp_16384x1_artix7 #(
    ...
    input  wire                  WREN,
    input  wire  [ 0:0]          WE,
    input  wire  [13:0]          RDADDR,
    input  wire  [13:0]          WRADDR,
    ...
    generate
        if (DATA_WIDTH != 1) begin : gen_error
            $fatal(1, "Error: DATA_WIDTH = %0d is out of range [1] for bram_sdp_16384x1_artix7.", DATA_WIDTH);
        end
        if (WRITE_MODE != "WRITE_FIRST" && WRITE_MODE != "READ_FIRST") begin : gen_write_mode_error
            $fatal(1, "Error: WRITE_MODE = %s is invalid (must be \"WRITE_FIRST\" or \"READ_FIRST\") for bram_sdp_16384x1_artix7.", WRITE_MODE);
        end
    endgenerate
