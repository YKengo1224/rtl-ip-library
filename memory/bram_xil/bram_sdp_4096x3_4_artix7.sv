...
module bram_sdp_4096x3_4_artix7 #(
    ...
    generate
        if (DATA_WIDTH < 3 || DATA_WIDTH > 4) begin : gen_error
            $fatal(1, "Error: DATA_WIDTH = %0d is out of range [3-4] for bram_sdp_4096x3_4_artix7.", DATA_WIDTH);
        end
        if (WRITE_MODE != "WRITE_FIRST" && WRITE_MODE != "READ_FIRST") begin : gen_write_mode_error
            $fatal(1, "Error: WRITE_MODE = %s is invalid (must be \"WRITE_FIRST\" or \"READ_FIRST\") for bram_sdp_4096x3_4_artix7.", WRITE_MODE);
        end
    endgenerate
