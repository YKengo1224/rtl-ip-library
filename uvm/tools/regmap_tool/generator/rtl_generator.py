...
class RtlGenerator(BaseGenerator):
    def __init__(self, model: RegmapModel, template_filename: str, module_name: str):
...
"""
                ...
                if acc != "WO":
                    ...
                    if field.source is not None and field.mask is not None:
...
"""        
        ...
        with open(self.template_filename, 'r', encoding='utf-8') as f:
            ...
            final_rtl = template_str.replace('uart_axilite_slv', f"{self.module_name}_axilite_slv")
            final_rtl = final_rtl.replace('// __INSERT_PORTS__', "\n".join(self.rtl_ports))
