...
module bram_sdp_8192x2_artix7 #(
    ...
    generate
        if (DATA_WIDTH != 2) begin : gen_error
            $fatal(1, "Error: DATA_WIDTH = %0d is out of range [2] for bram_sdp_8192x2_artix7.", DATA_WIDTH);
        end
        if (WRITE_MODE != "WRITE_FIRST" && WRITE_MODE != "READ_FIRST") begin : gen_write_mode_error
            $fatal(1, "Error: WRITE_MODE = %s is invalid (must be \"WRITE_FIRST\" or \"READ_FIRST\") for bram_sdp_8192x2_artix7.", WRITE_MODE);
        end
    endgenerate
