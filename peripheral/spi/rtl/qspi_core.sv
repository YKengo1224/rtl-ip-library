...
module qspi_core #(
        ...
        if (!qspi_ctrl_qspi_enable_sysclk_i || (slave_detect & qspi_csn_in_sysclk_i)) begin
            ...
            case (state)
                ...
                S_SEND: begin
                    ...
                    end else if(master_detect && finish_trans_sysclk_r && !tx_data_next_valid_sysclk_r) begin
                        state_next = S_IDLE;
                    end
                end
                S_SEND_FETCH: begin
    ...
    assign finish_trans_sysclk = (sample_edge_sysclk_r && (trans_bit_counter_sysclk_r == (max_trans_bit_counter_sysclk_r)));

    always_ff @(posedge sysclk or negedge srstn_sysclk) begin
        if (!srstn_sysclk) begin
            finish_trans_sysclk_r <= 1'b0;
        end else begin
            finish_trans_sysclk_r <= finish_trans_sysclk;
        end
    end
    ...
    always_ff @(posedge sysclk or negedge srstn_sysclk) begin
        if (!srstn_sysclk) begin
            ...
            if (finish_trans_sysclk_r && tx_data_next_valid_sysclk_r) begin
                tx_data_next_valid_sysclk_r <= 1'b0;
            end
        end else begin
            tx_data_next_valid_sysclk_r <= 1'b0;
    ...
    always_ff @(posedge sysclk or negedge srstn_sysclk) begin
        if (!srstn_sysclk) begin
            ...
            if (finish_trans_sysclk_r) begin
        ...
        if (!srstn_sysclk) begin
            ...
            case (qspi_ctrl_protocol_sel_sysclk_i)
                SINGLE_SPI_MODE: begin
                    ...
                    end else if (trans_bit_counter_sysclk_r == 0) begin
                        qspi_data_out_sysclk_o_r[0] <= '0;
                    end else if (qspi_ctrl_order_sysclk_i) begin
                        qspi_data_out_sysclk_o_r[0] <= tx_data_sysclk_r[(trans_bit_counter_sysclk_r-1)];
                    end else begin
                        qspi_data_out_sysclk_o_r[0] <= tx_data_sysclk_r[(qspi_ctrl_word_width_sysclk_i - 1) - (trans_bit_counter_sysclk_r-1)];
                    end
                end
                DUAL_SPI_MODE: begin
                    ...
                    if (qspi_ctrl_trans_dir_sysclk_i) begin
                        ...
                        if (trans_bit_counter_sysclk_r == 0) begin
                            qspi_data_out_sysclk_o_r[1:0] <= '0;
                        end else if (qspi_ctrl_order_sysclk_i) begin
                            qspi_data_out_sysclk_o_r[1:0] <= tx_data_sysclk_r[((trans_bit_counter_sysclk_r-1) << 1) +: 2];
                        end else begin
                            qspi_data_out_sysclk_o_r[1:0] <= tx_data_sysclk_r[8 - ((trans_bit_counter_sysclk_r-1) << 1) -1  -: 2];
                        end
                    end else begin
                        qspi_data_out_sysclk_o_r[1:0] <= '0;
                        qspi_data_out_en_sysclk_o_r   <= '0;
                ...
                QUAD_SPI_MODE: begin
                    if (qspi_ctrl_trans_dir_sysclk_i) begin
                        if (trans_bit_counter_sysclk_r == 0) begin
                            ...
                            qspi_data_out_sysclk_o_r[3:0] <= tx_data_sysclk_r[((trans_bit_counter_sysclk_r-1) << 2) +: 4];
                        end else begin
                            qspi_data_out_sysclk_o_r[3:0] <= tx_data_sysclk_r[8 - ((trans_bit_counter_sysclk_r-1) << 2) -1  -: 4];
                        end
    ...
    // rx data serial to pararel
    //-----------------------------------
    logic [4:0] latch_delay_count_r;
    logic [4:0] latch_delay_count_next;
    logic [4:0] latch_delay_count_max;
    logic [4:0] finish_trans_delay_count_r;
    logic [4:0] finish_trans_delay_count_next;

    logic       rx_data_latch_en;
    logic       finish_latch_sysclk;
    wire        single_spi_rx_bit_sysclk;

    assign latch_delay_count_max = qspi_ctrl_rx_latch_delay_sysclk_i + SYNC_FF_DEPTH[4:0];

    always_comb begin
        if (latch_delay_count_max == 5'd0) begin
            rx_data_latch_en = sample_edge_sysclk_r;
            finish_latch_sysclk = finish_trans_sysclk_r;
        end else begin
            rx_data_latch_en = (latch_delay_count_r == latch_delay_count_max);
            finish_latch_sysclk = (finish_trans_delay_count_r == latch_delay_count_max);
        end
    end
    ...
    always_comb begin
        if (sample_edge_sysclk_r && (latch_delay_count_max != 5'd0)) begin
            ...
            if (rx_data_latch_en) begin
                latch_delay_count_next = 5'd0;
            end else begin
                latch_delay_count_next = latch_delay_count_r + 5'd1;
            end
        end else begin
            latch_delay_count_next = 5'd0;
        end
    ...
    always_comb begin
        if (finish_trans_sysclk_r && (latch_delay_count_max != 5'd0)) begin
            finish_trans_delay_count_next = 5'd1;
        end else if (finish_trans_delay_count_r != 5'd0) begin
            if (finish_latch_sysclk) begin
                finish_trans_delay_count_next = 5'd0;
            end else begin
                finish_trans_delay_count_next = finish_trans_delay_count_r + 4'd1;
            end
        end else begin
            finish_trans_delay_count_next = 5'd0;
        end
    ...
    always_ff @(posedge sysclk or negedge srstn_sysclk) begin
        if (!srstn_sysclk) begin
            latch_delay_count_r <= 5'd0;
            finish_trans_delay_count_r <= 5'd0;
        end else begin
            latch_delay_count_r <= latch_delay_count_next;
            finish_trans_delay_count_r <= finish_trans_delay_count_next;
        end
    end


    assign single_spi_rx_bit_sysclk = (qspi_ctrl_spi_slave_en_sysclk_i)? qspi_data_in_sysclk_i[0] : qspi_data_in_sysclk_i[1];
    ...
    always_ff @(posedge sysclk or negedge srstn_sysclk) begin
        if (!srstn_sysclk) begin
            ...
            if (finish_latch_sysclk) begin
                rx_data_sysclk_r <= '0;
            end else if (rx_data_latch_en) begin

                case (qspi_ctrl_protocol_sel_sysclk_i)
                    SINGLE_SPI_MODE: begin
                        if (qspi_ctrl_order_sysclk_i) begin
                            rx_data_sysclk_r <= {single_spi_rx_bit_sysclk, rx_data_sysclk_r[15:1]};
                        end else begin
                            rx_data_sysclk_r <= {rx_data_sysclk_r[14:0], single_spi_rx_bit_sysclk};
                        end
                    end
                    DUAL_SPI_MODE: begin
                        if (qspi_ctrl_trans_dir_sysclk_i == '0) begin
                            if (qspi_ctrl_order_sysclk_i) begin
                                rx_data_sysclk_r <= {
        ...
        if (!srstn_sysclk) begin
            ...
            case (qspi_ctrl_protocol_sel_sysclk_i)
                SINGLE_SPI_MODE: begin
                    ...
                    if (qspi_ctrl_order_sysclk_i) begin
                        qspi_rx_fifo_data_in_sysclk_o_r <= rx_data_sysclk_r[15:0] >> (16 - qspi_ctrl_word_width_sysclk_i);
                    end else begin
                        qspi_rx_fifo_data_in_sysclk_o_r <= rx_data_sysclk_r[15:0];
                    end
                end
                DUAL_SPI_MODE, QUAD_SPI_MODE: begin
                    if (qspi_ctrl_trans_dir_sysclk_i == 2'b0) begin
                        qspi_rx_fifo_w_en_sysclk_o_r <= 1'b1;
                        qspi_rx_fifo_data_in_sysclk_o_r <= {8'd0, rx_data_sysclk_r[7:0]};
                    end else begin
                        qspi_rx_fifo_w_en_sysclk_o_r <= 1'b0;
