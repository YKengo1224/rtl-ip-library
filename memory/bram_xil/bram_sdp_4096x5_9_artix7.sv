...
module bram_sdp_4096x5_9_artix7 #(
    ...
    generate
        if (DATA_WIDTH < 5 || DATA_WIDTH > 9) begin : gen_error
            $fatal(1, "Error: DATA_WIDTH = %0d is out of range [5-9] for bram_sdp_4096x5_9_artix7.", DATA_WIDTH);
        end
        if (WRITE_MODE != "WRITE_FIRST" && WRITE_MODE != "READ_FIRST") begin : gen_write_mode_error
            $fatal(1, "Error: WRITE_MODE = %s is invalid (must be \"WRITE_FIRST\" or \"READ_FIRST\") for bram_sdp_4096x5_9_artix7.", WRITE_MODE);
        end
    endgenerate
